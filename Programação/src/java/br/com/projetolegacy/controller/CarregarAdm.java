 ...
 * @author Aluno
 */
@WebServlet(name = "CarregarAdm", urlPatterns = {"/CarregarAdm"})
public class CarregarAdm extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Integer idAdm = Integer.parseInt(request.getParameter("idAdm"));

        try {
            GenericDAO dao = new AdmDAOImpl();
            request.setAttribute("adm", dao.carregar(idAdm));
            request.getRequestDispatcher("ListarAdm").forward(request, response);

        } catch (Exception ex) {
            System.out.println("Erro no servlet CarregarADM: " + ex.getMessage());
            ex.printStackTrace();
